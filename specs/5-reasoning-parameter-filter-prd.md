...
- When proxy builds request for upstream, strip `reasoning` key at the outermost payload level; tolerate any truthy/falsy value types.
...
## TDD & Testing Plan
- Add unit test (fail first) for request builder ensuring payload missing `reasoning` before hitting HTTP layer.
- Add integration-style test with mocked upstream verifying request body matches expected sans `reasoning`.
